<?php

namespace App\Special\Tools;

/**
* Class for localization of values
*
* Класс для локализации значений
*
* @author Ilia Terebenin
*/
class Localizator
{
	/**
	 * Writes a string of russian words in Latin letters
	 * 
	 * Пишет строку из русских слов латинскими буквами
	 * 
	 * @param string $s
	 * @return string
	*/
    public static function translitString($s) {
		$s = (string) $s; 
		$s = strip_tags($s);
		$s = str_replace(array("\n", "\r"), " ", $s);
		$s = preg_replace("/\s+/", ' ', $s);
		$s = trim($s); 
		$s = function_exists('mb_strtolower') ? mb_strtolower($s) : strtolower($s); 
		$s = strtr($s, array('а'=>'a','б'=>'b','в'=>'v','г'=>'g','д'=>'d','е'=>'e','ё'=>'e','ж'=>'j','з'=>'z','и'=>'i','й'=>'y','к'=>'k','л'=>'l','м'=>'m','н'=>'n','о'=>'o','п'=>'p','р'=>'r','с'=>'s','т'=>'t','у'=>'u','ф'=>'f','х'=>'h','ц'=>'c','ч'=>'ch','ш'=>'sh','щ'=>'shch','ы'=>'i','э'=>'e','ю'=>'yu','я'=>'ya','ъ'=>'','ь'=>''));
		$s = preg_replace("/[^0-9a-z-_ ]/i", "", $s); 
		$s = str_replace(" ", "_", $s); 
		return $s; 
	}

}
